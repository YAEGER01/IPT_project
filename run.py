...
def update_profile():
    ...
    if request.method == "POST":
        ...
        cursor.execute("SELECT * FROM users WHERE email = %s AND id != %s",
                       (email, user_id))
        existing_user = cursor.fetchone()
        if existing_user:
        ...
        if password and confirm_password:
            ...
            cursor.execute("UPDATE users SET password=%s WHERE id=%s",
                           (hashed_password, user_id))
            flash(f"Your new password is: {generated_password}",
                  "info")  # You may want to securely notify the user
...
def student_dashboard():
    ...
    try:
        ...
        # Get student info
        cursor.execute("SELECT * FROM students WHERE user_id = %s",
                       (user_id, ))
